          	...
          	<div class="col-md-6">
				<div class="form-group">
					...
					<select name="status" id="status" class="custom-select custom-select-sm">
						...
						<option value="5" <?php echo isset($status) && $status == 5 ? 'selected' : '' ?>>Harvested</option>
					</select>
				</div>
			</div>
		</div>
      ...
      <?php endif; ?>
    	...
    	<div class="card-footer border-top border-info">
    		<div class="d-flex w-100 justify-content-center align-items-center">
    			...
    			<button class="btn btn-flat bg-gradient-secondary mx-2" type="button" onclick="location.href='index.php?page=view_project&id=<?php echo $_GET['id'] ?>'">Cancel</button>
    		</div>
    	</div>
	</div>
</div>
		...
		$.ajax({
			...
			success:function(resp){
				if(resp == 1){
					...
					setTimeout(function(){
						location.href = "index.php?page=view_project&id=<?php echo $_GET['id']?>"
					},2000)
				}
			}
		})

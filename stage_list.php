		...
		<div class="card-body">
			<table class="table tabe-hover table-condensed" id="list">
				...
				<tbody>
					...
					$qry = $conn->query("SELECT t.*,p.name as pname,p.start_date,p.status as pstatus, p.end_date,p.id as pid FROM stage_list t inner join project_list p on p.id = t.project_id $where order by p.name asc");
					while($row= $qry->fetch_assoc()):
						$trans = get_html_translation_table(HTML_ENTITIES,ENT_QUOTES);
						unset($trans["\""], $trans["<"], $trans[">"], $trans["<h2"]);
						$desc = strtr(html_entity_decode($row['description']),$trans);
						$desc=str_replace(array("<li>","</li>"), array("",", "), $desc);
						$tprog = $conn->query("SELECT * FROM stage_list where project_id = {$row['pid']}")->num_rows;
						$height = strtr(html_entity_decode($row['Height']),$trans);
						$height=str_replace(array("<li>","</li>"), array("",", "), $height);
		                $cprog = $conn->query("SELECT * FROM stage_list where project_id = {$row['pid']} and status = 3")->num_rows;
					...
					<tr>
						...
						<td>
							<p><b><?php echo ucwords($row['stage']) ?></b></p>
							<p class="truncate"><?php echo strip_tags($desc) ?></p>
							<p class="truncate"><?php echo strip_tags($height) ?></p>
						</td>
							...
							<button type="button" class="btn btn-default btn-sm btn-flat border-info wave-effect text-info dropdown-toggle" data-toggle="dropdown" aria-expanded="true">
		                      Action
			                    ...
			                    <div class="dropdown-menu" style="">
			                      <a class="dropdown-item new_productivity" data-pid = '<?php echo $row['pid'] ?>' data-tid = '<?php echo $row['id'] ?>'  data-stage = '<?php echo ucwords($row['stage']) ?>'  href="javascript:void(0)">Add Productivity</a>
								</div>
						</td>
					</tr>	
				<?php endwhile; ?>
...
<script>
	...
	$('.new_productivity').click(function(){
		uni_modal("<i class='fa fa-plus'></i> New Progress for: "+$(this).attr('data-stage'),"manage_progress.php?pid="+$(this).attr('data-pid')+"&tid="+$(this).attr('data-tid'),'large')
	})
	})

...
Class Action {
	...
	function save_stage(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO stage_list set $data");
		}else{
			$save = $this->db->query("UPDATE stage_list set $data where id = $id");
		}
		if($save){
			return 1;
		}
	...
	function delete_stage(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM stage_list where id = $id");
		if($delete){
			return 1;
		}
	}
